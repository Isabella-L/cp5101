        ...
        for stealth_loss in cfg.stealth_losses:
            for d_thr in cfg.d_threshes:
                for classifier_name in cfg.classifier_names:
                    ...
                    # targeted attack (batched)  # skipped for now
                    ...
                    if dl_based:
                        # created for only untargeted attacks for now
                        ut.save_imgs(expand_4d(cam_infer_adv_untar), cam_infer_adv_path)
                        ut.save_imgs(expand_4d(prj_adv_untar), prj_adv_path)
...
def spaa(pcnet, classifier, imagenet_labels, target_idx, targeted, cam_scene, d_thr, stealth_loss, device, setup_info):
    ...
    # learning rateswecin minimizing adversarial loss
