...
def init_visdom(server="localhost", port=8097):
    ...
    vis = visdom.Visdom(
        server=server, port=port, use_incoming_socket=False
    )  # default is 8097
...
class SimpleDataset(Dataset):
    ...
    def __init__(self, data_root, index=None, size=None):
        ...
        if index is not None:
            img_list = [img_list[x] for x in index]
...
def torch_imread(filename):
    return (
        torch.Tensor(
            cv.cvtColor(cv.imread(filename), cv.COLOR_BGR2RGB).transpose((2, 0, 1))
        )
        / 255
    )
...
def save_imgs(im_4d, path, idx=0):
    ...
    if type(im_4d) is torch.Tensor:
        ...
        if im_4d.device.type == "cuda":
            imgs = (
                im_4d.cpu().numpy().transpose(0, 2, 3, 1)
            )  # (N, C, row, col) to (N, row, col, C)
        else:
            imgs = im_4d.numpy().transpose(
                0, 2, 3, 1
            )  # (N, C, row, col) to (N, row, col, C)
    else:
        imgs = im_4d
    ...
    if imgs.dtype == "float32":
        imgs = np.uint8(
            imgs[:, :, :, ::-1] * 255
        )  # convert to BGR and uint8 for opencv
    else:
...
def vis_pcnet_process(pcnet, prj_input, cam_scene, cam_train, cam_mask, cp_sz):
    ...
    with torch.no_grad():
        ...
        prj_warp_masked = torch.where(cam_mask, prj_warp.cpu(), torch.tensor([0.0])).to(
            device
        )
    ...
    im_montage = make_grid_transposed(
        torch.cat(
            (
                cam_scene,
                cam_mask_vis,
                resize(prj_input, cp_sz),
                resize(prj_warp, cp_sz),
                prj_warp_masked,
                cam_rough,
                cam_infer,
                cam_train,
...
def plot_montage(
    *argv, index=None, win=None, title=None, env=None, grid_w=5, cp_sz=None
):
    """
    ...
    with torch.no_grad():  # just in case
        ...
        for im in argv:
            if im.shape[2] != tile_size[0] or im.shape[3] != tile_size[1]:
                if cp_sz is not None:
                    im_resize[i : i + grid_w] = F.interpolate(
                        cc(im[idx, :, :, :], cp_sz), tile_size
                    )
                else:
                    im_resize[i : i + grid_w] = F.interpolate(
                        im[idx, :, :, :], tile_size
        ...
        plot_opts = dict(
            title=title, caption=title, font=dict(size=18), tore_history=False
        )
...
def montage(im_in, grid_shape=None, padding_width=5, fill=(1, 1, 1), multichannel=True):
    ...
    if type(im_in) is np.ndarray:
        ...
        assert (
            im_in.ndimension() == 4
        ), "requires a 4-D tensor with shape (N, C, row, col)"
...
def rmse(x, y):
    ...
    with torch.no_grad():
        ...
        return math.sqrt(
            l2_fun(x, y).item() * 3
        )  # only works for RGB, for grayscale, don't multiply by 3
...
def project_capture_data(prj_input_path, cam_cap_path, setup_info):
    ...
    for i in tqdm(range(num_im)):
        ...
        cv.imwrite(
            ...
            cv.resize(cc(im_cam, cam_crop_sz), cam_im_sz, interpolation=cv.INTER_AREA),
        )
...
def write_log_file(df, log_txt_filename=None, log_xls_filename=None, mode="w"):
    ...
    if log_xls_filename is not None:
        df.to_excel(
            log_xls_filename, index=False, float_format="%.4f"
        )  # can only overwrite
