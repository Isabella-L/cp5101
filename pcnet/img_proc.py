...
def checkerboard(*args):
    ...
    if q % 2 == 0:
        ...
        checkerboard[:, mid_col:] = checkerboard[:, mid_col:] - 0.3
...
def insert_text(x, text, pos, font_sz=14, color=(0, 0, 0)):
    ...
    if sys.platform == "win32":
        ...
        font = ImageFont.truetype(
            "/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf", font_sz
        )
    else:
...
def to_pseudocolor(x, colormap=cv.COLORMAP_BONE):
    ...
    x = (torch.Tensor(cv.cvtColor(x, cv.COLOR_BGR2RGB).transpose((2, 0, 1))) / 255).to(
        device
    )
    return x
