...
# l2_fun = nn.MSELoss()
ssim_fun = (
    pytorch_ssim.SSIM().cuda() if torch.cuda.is_available() else pytorch_ssim.SSIM()
)
...
def load_data(data_root, setup_name, input_size=None, compensation=False):
    ...
    prj_valid = ut.torch_imread_mt(
        prj_valid_path, index=[i for i in range(cam_valid.shape[0])]
    )  # only read the same number of cam_valid
    ...
    im_mask, _, mask_corners = threshold_im(
        im_direct, compensation=compensation
    )  # use thresholded as mask
...
def load_setup_info(setup_path):
    ...
    if os.path.exists(setup_info_filename):
        ...
        warnings.warn(
            f"{setup_info_filename} not found, loading {setup_info_default_filename} instead"
        )
    return setup_info
...
def init_compennet(compennet, data_root, cfg):
    ...
    ckpt_file = join(
        data_root,
        "../checkpoint/init_CompenNet_l1+ssim_500_48_500_0.001_0.2_800_0.0001.pth",
    )
...
def train_compennet_pp(model, train_data, valid_data, cfg):
    ...
    lr_scheduler = optim.lr_scheduler.StepLR(
        optimizer, step_size=cfg["lr_drop_rate"], gamma=cfg["lr_drop_ratio"]
    )
    ...
    if "model_name" not in cfg:
        cfg["model_name"] = model.name if hasattr(model, "name") else model.module.name

    # initialize visdom data visualization figure
    if "plot_on" not in cfg:
        cfg["plot_on"] = True
    ...
    while iters < cfg["max_iters"]:
        ...
        cam_train_batch = (
            cam_train[idx].to(device)
            if cam_train.device.type != "cuda"
            else cam_train[idx]
        )
        prj_train_batch = (
        ...
        train_loss_batch, train_l2_loss_batch = compute_loss(
            prj_train_infer, prj_train_batch, cfg["loss"]
        )
        ...
        if cfg["plot_on"]:
            ...
            if iters % cfg["train_plot_rate"] == 0 or iters == cfg["max_iters"] - 1:
                vis_train_fig = plot_montage(
                    cam_train_batch[vis_idx],
                    prj_train_infer[vis_idx],
                    prj_train_batch[vis_idx],
                    win=vis_train_fig,
                    title="[Train]" + title,
                )
                append_data_point(
        ...
        if valid_data is not None and (
            iters % cfg["valid_rate"] == 0 or iters == cfg["max_iters"] - 1
        ):
            valid_psnr, valid_rmse, valid_ssim, prj_valid_infer = evaluate_model(
                model, valid_data
            )
            ...
            if cfg["plot_on"]:
                ...
                vis_valid_fig = plot_montage(
                    valid_data["cam_valid"][idx],
                    prj_valid_infer[idx],
...
def train_pcnet(model, train_data, valid_data, cfg):
    ...
    aff_tps_params = list(
        map(
            ...
            list(
                filter(
                    lambda kv: kv[0]
                    in ["module.warping_net.affine_mat", "module.warping_net.theta"],
                    model.named_parameters(),
                )
            ),
        )
    )
    refinenet_params = list(
        map(
            lambda x: x[1],
            list(
                filter(
                    lambda kv: "module.warping_net.grid_refine_net" in kv[0],
                    model.named_parameters(),
                )
            ),
        )
    )
    shadingnet_params = list(
        map(
            lambda x: x[1],
            list(
                filter(
                    lambda kv: "module.warping_net" not in kv[0],
                    model.named_parameters(),
                )
            ),
        )
    ...
    if 1:
        ...
        w2_optimizer = optim.Adam(
            [{"params": refinenet_params}], lr=5e-3, weight_decay=0
        )
        s_optimizer = optim.Adam(
            [{"params": shadingnet_params}], lr=1e-3, weight_decay=cfg.l2_reg
        )
    else:
        # for pretrained PCNet finetune
        w1_optimizer = optim.Adam([{"params": aff_tps_params}], lr=1e-3, weight_decay=0)
        w2_optimizer = optim.Adam(
            [{"params": refinenet_params}], lr=1e-2, weight_decay=0
        )
        s_optimizer = optim.Adam(
            [{"params": shadingnet_params}], lr=1e-4, weight_decay=cfg.l2_reg
        )

    # learning rate drop scheduler
    w1_lr_scheduler = optim.lr_scheduler.MultiStepLR(
        w1_optimizer, milestones=[100], gamma=cfg.lr_drop_ratio
    )
    w2_lr_scheduler = optim.lr_scheduler.MultiStepLR(
        w2_optimizer, milestones=[1200], gamma=cfg.lr_drop_ratio
    )
    s_lr_scheduler = optim.lr_scheduler.MultiStepLR(
        s_optimizer, milestones=[1800], gamma=cfg.lr_drop_ratio
    )
    ...
    while iters < cfg.max_iters:
        ...
        train_loss_batch, train_l2_loss_batch = compute_loss(
            cam_train_infer, cam_train_batch, cfg.loss
        )
        train_rmse_batch = math.sqrt(train_l2_loss_batch.item() * 3)  # 3 channel, rgb
        ...
        if valid_data is not None and (
            ...
            valid_psnr, valid_rmse, valid_ssim, cam_valid_infer = evaluate_model(
                model, valid_data
            )
            ...
            if cfg.plot_on:
                ...
                vis_valid_fig = plot_montage(
                    valid_data["prj_valid"][vis_idx],
                    cam_valid_infer[vis_idx],
                    valid_data["cam_valid"][vis_idx],
                    win=vis_valid_fig,
                    title="[Valid]" + title,
                )
                append_data_point(iters, valid_rmse, vis_curve_fig, "valid_rmse")
...
def evaluate_model(model, valid_data, chunk_sz=10):
    ...
    with torch.no_grad():
        ...
        batch_idx = torch.chunk(
            torch.arange(num_valid), chunk_sz
        )  # chunks = num_valid sets batch size to 1

        for idx in batch_idx:
            batch_size = len(idx)
            cam_scene_batch = (
                cam_scene[idx].to(device)
                if cam_scene.device.type != "cuda"
                else cam_scene[idx]
            )
            cam_valid_batch = (
                cam_valid[idx].to(device)
                if cam_valid.device.type != "cuda"
                else cam_valid[idx]
            )
            prj_valid_batch = (
            ...
            if type(model_infer_batch) == tuple and len(model_infer_batch) > 1:
                model_infer_batch = model_infer_batch[0]
...
def get_model_train_cfg(
    ...
    cfg_default.lr_drop_rate = (
        800  # TODO PCNet uses its own lr_drop_rate, and this is ignored
    )
    ...
    cfg_default.center_crop = (
        center_crop  # whether to center crop the training/validation images
    )
...
def train_eval_pcnet(cfg_default):
    ...
    # log
    ret, log_txt_filename, log_xls_filename = ut.init_log_file(
        join(data_root, "../log")
    )
    ...
    for setup_name in cfg_default.setup_list:
        ...
        (
            ...
            mask_corners,
            setup_info,
        ) = load_data(data_root, setup_name)
        ...
        valid_data = dict(
            cam_scene=cam_scene_valid, cam_valid=cam_valid, prj_valid=prj_valid
        )
        ...
        for num_train in cfg_default.num_train_list:
            ...
            for model_name in cfg_default.model_list:
                ...
                for loss in cfg_default.loss_list:
                    ...
                    shading_net = models.ShadingNetSPAA(
                        use_rough="no_rough" not in model_name
                    )
                    if torch.cuda.device_count() >= 1:
                        shading_net = nn.DataParallel(
                            shading_net, device_ids=cfg.device_ids
                        ).to(device)

                    # create a WarpingNet model
                    warping_net = models.WarpingNet(
                        out_size=cam_train.shape[-2:],
                        with_refine="w/o_refine" not in model_name,
                    )  # warp prj to cam raw
                    ...
                    affine_mat = torch.Tensor(
                        cv.getAffineTransform(dst_pts, src_pts)
                    )  # prj -> cam
                    warping_net.set_affine(affine_mat.flatten())
                    if torch.cuda.device_count() >= 1:
                        warping_net = nn.DataParallel(
                            warping_net, device_ids=cfg.device_ids
                        ).to(device)
                    ...
                    pcnet = models.PCNet(
                        ...
                        use_mask="no_mask" not in model_name,
                        use_rough="no_rough" not in model_name,
                    )
                    if torch.cuda.device_count() >= 1:
                        pcnet = nn.DataParallel(pcnet, device_ids=cfg.device_ids).to(
                            device
                        )
                    ...
                    if not cfg.load_pretrained:
                        print(
                            f"------------------------------------ Start training {model_name:s} ---------------------------"
                        )
                        pcnet, valid_psnr, valid_rmse, valid_ssim = train_pcnet(
                            pcnet, train_data, valid_data, cfg
                        )
                    else:
                        # load the previously trained PCNet instead of retraining a new one
                        print(
                            f"------------------------------------ Loading pretrained {model_name:s} ---------------------------"
                        )
                        checkpoint_filename = join(
                            data_root, "../checkpoint", ut.opt_to_string(cfg) + ".pth"
                        )
                    ...
                    (
                        ...
                        valid_linf,
                        valid_dE,
                    ) = ut.calc_img_dists(cam_valid_infer, cam_valid)
                    ...
                    ut.write_log_file(
                        ret, log_txt_filename, log_xls_filename
                    )  # in case unexpected interruptions, we save logs for each setup

                    # save inferred camera-captured (relit) images
                    setup_path = join(data_root, "setups", setup_name)
                    cam_valid_infer_path = join(
                        setup_path, "cam/infer/test", model_version
                    )
                    ut.save_imgs(cam_valid_infer.detach().cpu(), cam_valid_infer_path)
                    print(
                        "Inferred camera-captured (relit) images saved to "
                        + cam_valid_infer_path
                    )
                    print(
                        "------------------------------------ Done! ---------------------------\n"
                    )
    ...
    for model_name in cfg_default.model_list:
        ret.loc[len(ret)] = ret.loc[ret["Model"] == model_name].mean(
            axis=0, numeric_only=True
        )
        ret.loc[len(ret) - 1, ["Setup", "Model"]] = [
            f"[mean]_{len(cfg_default.setup_list)}_setups",
            model_name,
        ]
...
def train_eval_compennet_pp(cfg_default):
    ...
    if torch.cuda.device_count() >= 1:
        compen_net = nn.DataParallel(compen_net, device_ids=cfg_default.device_ids).to(
            device
        )
        ...
        for num_train in cfg_default.num_train_list:
            ...
            for model_name in cfg_default.model_list:
                ...
                for loss in cfg_default.loss_list:
                    ...
                    # create a WarpingNet model
                    warping_net = models.WarpingNet(
                        out_size=prj_size, with_refine="w/o_refine" not in model_name
                    )  # warp prj to cam raw
                    ...
                    if torch.cuda.device_count() >= 1:
                        compennet_pp = nn.DataParallel(
                            compennet_pp, device_ids=cfg.device_ids
                        ).to(device)
                    ...
                    if not cfg.load_pretrained:
                        ...
                        compennet_pp, valid_psnr, valid_rmse, valid_ssim = (
                            train_compennet_pp(
                                compennet_pp, train_data, valid_data, cfg
                            )
                        )
                    else:
                        # load the previously trained PCNet instead of retraining a new one
                    ...
                    (
                        ...
                        valid_linf,
                        valid_dE,
                    ) = ut.calc_img_dists(prj_valid_infer, prj_valid)
                    ...
                    prj_valid_infer_path = join(
                        setup_path, "prj/infer/test", model_version
                    )
                    ut.save_imgs(prj_valid_infer, prj_valid_infer_path)
                    print(
                        "Inferred projector input validation images saved to "
                        + prj_valid_infer_path
                    )

                    # [testing phase] create compensated test images for real compensation (to project and capture)
                    print(
                        f"------------------------------------ Saving compensated test images for {model_name:s} ---------------------------"
                    )
                    ...
                    if os.path.isdir(desire_test_path):
                        ...
                        with torch.no_grad():
                            # simplify CompenNet++
                            compennet_pp.module.simplify(
                                cam_scene_test[0, ...].unsqueeze(0)
                            )

                            # compensate using CompenNet++
                            compennet_pp.eval()
                            prj_cmp_test = compennet_pp(
                                desire_test, cam_scene_test
                        ...
                        warnings.warn(
                            f"images and folder {desire_test_path:s} does not exist, no compensation images saved!"
                        )
                    print(
                        "------------------------------------ Done! ---------------------------\n"
                    )
